...
`main` carries released code and moves when a version is cut; work in progress
lives on branches. Issues and pull requests are welcome, and the one thing worth
knowing before opening either is that `scripts/check.sh` is the whole bar:
format, lints with warnings as errors, build, tests, documentation and the
registry archive. `make dev` runs it.
